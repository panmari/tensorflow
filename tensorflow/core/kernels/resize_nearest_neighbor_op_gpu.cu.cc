...
__global__ void ResizeNearestNeighborNCHW(const int nthreads, const dtype* bottom_data,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    int dest_x = index % dest_width;
    int dest_y = (index / dest_width) % dest_height;
    int c = (index / dest_width / dest_height) % channels;
    int n = index / dest_width / dest_height / channels;

    const dtype* bottom_data_n = bottom_data + n * channels * height * width;
    const int in_x = std::min(static_cast<int64>(floorf(dest_x * width_scale)),
                                    (in_width - 1));
    const int in_y = std::min(static_cast<int64>(floorf(dest_y * height_scale)),
                              (in_height - 1));
    int idx = c * height * width + in_y * width + in_x;
